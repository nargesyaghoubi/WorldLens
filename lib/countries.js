...
// Fetch a single page of countries (max 100 per request, the API's limit).
async function fetchCountriesPage({ limit, offset, cache }) {
...
export async function getAllCountries({ cache = "force-cache" } = {}) {
    ...
    while (true) {
        const batch = await fetchCountriesPage({
            limit: PAGE_SIZE,
            offset,
            cache,
        });
